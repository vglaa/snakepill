...
export async function createPlayer(walletAddress, username = null) {
    const { data, error } = await supabaseAdmin
        .from('players')
        .insert({
...
export async function updatePlayer(walletAddress, updates) {
    const { data, error } = await supabaseAdmin
        .from('players')
...
export async function createGameSession(playerId, walletAddress) {
    const { data, error } = await supabaseAdmin
        .from('game_sessions')
        .insert({
            player_id: playerId,
...
export async function endGameSession(sessionId, score, playtimeSeconds, pillsEaten, reason) {
    const { data, error } = await supabaseAdmin
        .from('game_sessions')
        .update({
            score,
...
export async function addToLeaderboard(playerId, walletAddress, username, score, playtimeSeconds) {
    const { data, error } = await supabaseAdmin
        .from('leaderboard')
        .insert({
            player_id: playerId,
...
export async function setPlayerEligible(playerId, walletAddress, holdingUsd, totalPlaytime) {
    const { data, error } = await supabaseAdmin
        .from('eligible_players')
        .upsert({
            player_id: playerId,
...
export async function removePlayerEligibility(walletAddress) {
...
export async function updateOnlinePlayer(sessionId, walletAddress, isPlaying) {
    const { error } = await supabaseAdmin
        .from('online_players')
        .upsert({
...
export async function removeOnlinePlayer(sessionId) {
    const { error } = await supabaseAdmin
        .from('online_players')
        .delete()
...
export async function buySkin(walletAddress, skinId) {
    // Get player
...
export async function equipSkin(walletAddress, skinId) {
    const player = await getPlayer(walletAddress);
    if (!player) throw new Error('Player not found');
...
// Tax distribution log
export async function logTaxDistribution(totalTaxSol, distributionAmount, eligibleCount, perPlayerSol, txSignatures) {
    const { data, error } = await supabaseAdmin
        .from('tax_distributions')
        .insert({
            total_tax_sol: totalTaxSol,
            distribution_amount_sol: distributionAmount,
            eligible_count: eligibleCount,
            per_player_sol: perPlayerSol,
    ...
    await supabaseAdmin.rpc('update_total_distributed', { amount: distributionAmount });
...
export async function getPlayersWithMinPlaytime(minSeconds) {
    const { data, error } = await supabaseAdmin
        .from('players')
        .select('*')
